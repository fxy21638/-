...
**`_trim_silence()`** — 用 `librosa.effects.trim(top_db=28)` 裁掉首尾静音。预测流程在提取特征前调用，避免静音帧拉低谱质心等特征导致误判。变声流程在 WORLD 分析前调用，避免无声段产生杂音。

**关键函数调用链（预测）：**
1. `PredictWorker.run()` → VAD 检测（active_ratio > 10%）→ `_trim_silence()` 裁首尾静音 → `_ensure_min_duration()` → `_load_model()` → `extract_audio_features()` → XGBoost 推理 → 显示结果
